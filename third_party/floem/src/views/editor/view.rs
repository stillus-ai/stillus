...
impl View for EditorView {
    ...
    fn layout(&mut self, cx: &mut LayoutCx) -> crate::taffy::tree::NodeId {
        cx.layout_node(self.id, true, |_cx| {
            ...
            // Editor-width wrapping must not retain an earlier unwrapped intrinsic width.
            let width = if editor.es.with_untracked(|style| {
                matches!(style.wrap_method(), super::text::WrapMethod::EditorWidth)
            }) {
                parent_size.width()
            } else {
                editor.max_line_width().max(parent_size.width())
            };
